...
<br>**Answer**: Ethernet, IPv4, IPv6, SRv6, TCP, UDP, ICMP, ICMPv6, NDP
...
<br>**Answer**: l2_ternary_table uses ternary match to the pattern ``33:33:**:**:**:**``
where ``**`` means dont-care value.
* In the ACL table, what's the difference between `send_to_cpu` and
  `clone_to_cpu` actions
<br>**Answer**: `send_to_cpu` update the egress port and send a "Packet Out" packet out of the controller, `clone_to_cpu` takes a "Packet In", clone and send to controller, and let that packet forward to its egress port as normal.
* In the apply block, what is the first table applied to a packet? Are P4Runtime
  packet-out treated differently?
<br>**Answer**: l2_exact_table is applied first, and l2_ternary_table applied only if the exact table misses. And yes PacketOut from controller are treated differently. If `cpu_out` is set, copy `hdr.cpu_out.egress_port` into `standard_metadata.egress_spec`...

**Egress pipeline**

* For multicast packets, can they be replicated to the ingress port?
<br>**Answer**: No, we make sure not to replicate to the port where it's from for multicast packet.

**Deparser**

* What is the first header to be serialized on the wire and in which case?
<br>**Answer**: `cpu_in` contains the ingress port
...
1. What is the fully qualified name of the `l2_exact_table`? What is its numeric
   ID?
  <br>**Answer**: full name is `IngressPipeImpl.l2_exact_table` and ID 33605373
2. To which P4 entity does the ID `16812802` belong to? A table, an action, or
  ...
  <br>**Answer**: it corresponds to an action call `set_egress_port` with full qualified name `IngressPipeImpl.set_egress_port`
3. For the `IngressPipeImpl.set_egress_port` action, how many parameters are
   ...
   `port_num`?
  <br>**Answer**: 3 parameters: `id`, `name`, and `bitwidth`. `bitwidth` is set to 9
4. At the end of the file, look for the definition of the
  ...
  <br>**Answer**: Yes a description of the PacketOut header was implemented
