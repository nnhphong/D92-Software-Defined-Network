...
parser ParserImpl (packet_in packet,
                   out parsed_headers_t hdr,    // must write: packet.extract(hdr.<field>) populated header
                   inout local_metadata_t local_metadata, // can read & write
                   inout standard_metadata_t standard_metadata)
{
    state start {
        // start by looking at ingress port of the packet
        transition select(standard_metadata.ingress_port) {
            // controller PacketOuts are expected to have cpu_out header first
...
control IngressPipeImpl (inout parsed_headers_t    hdr,
    ...
    action ndp_ns_to_na(mac_addr_t target_mac) {
        ...
        hdr.ndp.flags = NDP_FLAG_ROUTER | NDP_FLAG_SOLICITED | NDP_FLAG_OVERRIDE;
